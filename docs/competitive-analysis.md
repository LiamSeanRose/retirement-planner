...
| **"When can I retire? How much can I spend?" — answered exactly** | ✅ **BUILT** — the two questions people most want, solved off the REAL engine (pension + CPP/OAS + tax + portfolio): max sustainable spend, earliest retirement age, and a returns-down sensitivity — not the rough 4%/25× rule. Surfaced as a "Your questions, answered" panel (`lib/analysis/solve`). |
| **More "ifs": benefit cut + move provinces** | ✅ **BUILT** — a policy-risk CPP/OAS cut from an age, and relocating to another province mid-retirement (provincial tax follows the move — leverages the verified 13-province tax). Guided-setup wizard also asks risk tolerance. |
