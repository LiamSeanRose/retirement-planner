...
interface ScenarioResult {        // result of ONE run over ONE path (deterministic, or the median under Monte Carlo)
  scenario: Scenario;
  ...
  monteCarlo?: MonteCarloResult;                     // populated in Monte Carlo mode (full bands below)
}
...
interface OptimizerRequest { objective: Objective; lockedLevers?: Partial<Scenario>; }
interface OptimizerResult { bestScenario: Scenario; objectiveValue: number; result: ScenarioResult; }
```
...
- **Phase 3 — Strategy modules + estate projector.** RRSP meltdown (OAS guard + TFSA pipeline), the §20 tax-optimized withdrawal order (non-reg → RRSP meltdown → TFSA last), CPP/OAS timing comparison, and the after-tax estate projection (terminal tax).
...
- **Phase 6 — Probabilistic analysis (§20).** Monte Carlo (probability of success + p5–p95 percentile fan charts) and the stress-test library, running in a Web Worker on a seedable RNG. Deterministic stays the default/fast path.
- **Phase 7 — Optimizers (§10)** (after Dad answers §17). CPP/OAS optimizer (exact enumeration) + withdrawal/strategy optimizer (knob search) with selectable objectives, scored against the deterministic median or the Monte Carlo probability-of-success.
- **Phase 8 — Conquest-style dashboard (§21).** Confidence-score dial, scenario-lab toggles, stacked cash-flow area chart to age 95. Read the `frontend-design` skill first.
- **Phase 9 — Polish + deploy.** `frontend-design` pass, disclaimers, mobile, a11y, Vercel deploy, add to portfolio.

Each phase ends runnable on localhost. Engine-first throughout: the analysis layer and UI consume only what the tested engine exposes. (Several Phase-1/analysis leaf modules — `accounts`, `tax`, `rng`, `paths`, `stress` — are being built in parallel ahead of the Phase-1 projection integration.)
